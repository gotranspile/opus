...
func silk_decode_indices(psDec *silk_decoder_state, psRangeDec *ec_dec, FrameIndex int, decode_LBRR int, condCoding int) {
	...
	if condCoding == CODE_CONDITIONALLY {
		...
		psDec.Indices.GainsIndices[0] = int8(int32(int(uint32(int32(ec_dec_icdf(psRangeDec, &silk_gain_iCDF[psDec.Indices.SignalType][0], 8)))) << 3))
	...
	psDec.Indices.NLSFIndices[0] = int8(ec_dec_icdf(psRangeDec, (*uint8)(unsafe.Add(unsafe.Pointer(psDec.PsNLSF_CB.CB1_iCDF), (int(psDec.Indices.SignalType)>>1)*int(psDec.PsNLSF_CB.NVectors))), 8))
	...
	if int(psDec.Indices.SignalType) == TYPE_VOICED {
		...
		if condCoding == CODE_CONDITIONALLY && psDec.Ec_prevSignalType == TYPE_VOICED {
			delta_lagIndex = int(int16(ec_dec_icdf(psRangeDec, &silk_pitch_delta_iCDF[0], 8)))
			if delta_lagIndex > 0 {
		...
		if decode_absolute_lagIndex != 0 {
			psDec.Indices.LagIndex = int16(int(int16(ec_dec_icdf(psRangeDec, &silk_pitch_lag_iCDF[0], 8))) * (psDec.Fs_kHz >> 1))
