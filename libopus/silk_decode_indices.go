...
func silk_decode_indices(psDec *silk_decoder_state, psRangeDec *ec_dec, FrameIndex int, decode_LBRR int, condCoding int) {
	...
	psDec.Indices.NLSFIndices[0] = int8(ec_dec_icdf(psRangeDec, []byte((*uint8)(unsafe.Add(unsafe.Pointer(psDec.PsNLSF_CB.CB1_iCDF), (int(psDec.Indices.SignalType)>>1)*int(psDec.PsNLSF_CB.NVectors)))), 8))
	silk_NLSF_unpack(ec_ix[:], pred_Q8[:], psDec.PsNLSF_CB, int(psDec.Indices.NLSFIndices[0]))
	for i = 0; i < int(psDec.PsNLSF_CB.Order); i++ {
		Ix = ec_dec_icdf(psRangeDec, []byte((*uint8)(unsafe.Add(unsafe.Pointer(psDec.PsNLSF_CB.Ec_iCDF), ec_ix[i]))), 8)
		if Ix == 0 {
	...
	if int(psDec.Indices.SignalType) == TYPE_VOICED {
		...
		if decode_absolute_lagIndex != 0 {
			...
			psDec.Indices.LagIndex += int16(ec_dec_icdf(psRangeDec, []byte(psDec.Pitch_lag_low_bits_iCDF), 8))
		}
		psDec.Ec_prevLagIndex = psDec.Indices.LagIndex
		psDec.Indices.ContourIndex = int8(ec_dec_icdf(psRangeDec, []byte(psDec.Pitch_contour_iCDF), 8))
