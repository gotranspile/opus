...
func silk_control_SNR(psEncC *silk_encoder_state, TargetRate_bps opus_int32) int64 {
	var (
		id        int64
		bound     int64
		snr_table *uint8
	...
	if psEncC.Nb_subfr == 2 {
		TargetRate_bps -= opus_int32(psEncC.Fs_kHz/16 + 2000)
	}
	if psEncC.Fs_kHz == 8 {
		bound = int64(107)
		snr_table = &silk_TargetRate_NB_21[0]
	} else if psEncC.Fs_kHz == 12 {
		bound = int64(155)
		snr_table = &silk_TargetRate_MB_21[0]
	} else {
		bound = int64(191)
		snr_table = &silk_TargetRate_WB_21[0]
	}
	id = int64((TargetRate_bps + 200) / 400)
	if (id - 10) < (bound - 1) {
		id = id - 10
	} else {
		id = bound - 1
	...
	if id <= 0 {
		...
		psEncC.SNR_dB_Q7 = int64(*(*uint8)(unsafe.Add(unsafe.Pointer(snr_table), id))) * 21
	}
