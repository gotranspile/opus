	...
	for i = int(order) - 1; i >= 0; i-- {
		...
		for j = 0; j < nStates; j++ {
			...
			if ind_tmp+1 >= NLSF_QUANT_MAX_AMPLITUDE {
				if ind_tmp+1 == NLSF_QUANT_MAX_AMPLITUDE {
					rate0_Q5 = int(*(*uint8)(unsafe.Add(unsafe.Pointer(rates_Q5), ind_tmp+NLSF_QUANT_MAX_AMPLITUDE)))
					rate1_Q5 = 280
				} else {
				...
				if ind_tmp == -NLSF_QUANT_MAX_AMPLITUDE {
					rate0_Q5 = 280
					rate1_Q5 = int(*(*uint8)(unsafe.Add(unsafe.Pointer(rates_Q5), ind_tmp+1+NLSF_QUANT_MAX_AMPLITUDE)))
				} else {
