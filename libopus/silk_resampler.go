...
func silk_resampler(S *silk_resampler_state_struct, out []int16, in []int16, inLen int32) int {
	...
	case 1:
		silk_resampler_private_up2_HQ_wrapper(unsafe.Pointer(S), &out[0], &S.DelayBuf[0], int32(S.Fs_in_kHz))
		silk_resampler_private_up2_HQ_wrapper(unsafe.Pointer(S), &out[S.Fs_out_kHz], &in[nSamples], int32(int(inLen)-S.Fs_in_kHz))
	case 2:
		silk_resampler_private_IIR_FIR(unsafe.Pointer(S), out, S.DelayBuf[:], int32(S.Fs_in_kHz))
		silk_resampler_private_IIR_FIR(unsafe.Pointer(S), []int16(&out[S.Fs_out_kHz]), []int16(&in[nSamples]), int32(int(inLen)-S.Fs_in_kHz))
	case 3:
		silk_resampler_private_down_FIR(unsafe.Pointer(S), out, S.DelayBuf[:], int32(S.Fs_in_kHz))
		silk_resampler_private_down_FIR(unsafe.Pointer(S), []int16(&out[S.Fs_out_kHz]), []int16(&in[nSamples]), int32(int(inLen)-S.Fs_in_kHz))
	default:
