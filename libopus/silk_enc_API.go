	...
	for {
		...
		if int(encControl.NChannelsAPI) == 2 && int(encControl.NChannelsInternal) == 2 {
			...
			for n = 0; n < nSamplesFromInput; n++ {
				*(*int16)(unsafe.Add(unsafe.Pointer(buf), unsafe.Sizeof(int16(0))*uintptr(n))) = *(*int16)(unsafe.Add(unsafe.Pointer(samplesIn), unsafe.Sizeof(int16(0))*uintptr(n*2)))
			}
			...
			for n = 0; n < nSamplesFromInput; n++ {
				*(*int16)(unsafe.Add(unsafe.Pointer(buf), unsafe.Sizeof(int16(0))*uintptr(n))) = *(*int16)(unsafe.Add(unsafe.Pointer(samplesIn), unsafe.Sizeof(int16(0))*uintptr(n*2+1)))
			}
			...
			for n = 0; n < nSamplesFromInput; n++ {
				sum = int32(int(*(*int16)(unsafe.Add(unsafe.Pointer(samplesIn), unsafe.Sizeof(int16(0))*uintptr(n*2)))) + int(*(*int16)(unsafe.Add(unsafe.Pointer(samplesIn), unsafe.Sizeof(int16(0))*uintptr(n*2+1)))))
				if 1 == 1 {
			...
			libc.MemCpy(unsafe.Pointer(buf), unsafe.Pointer(samplesIn), nSamplesFromInput*int(unsafe.Sizeof(int16(0))))
			ret += silk_resampler(&psEnc.State_Fxx[0].SCmn.Resampler_state, []int16(&psEnc.State_Fxx[0].SCmn.InputBuf[psEnc.State_Fxx[0].SCmn.InputBufIx+2]), []int16(buf), int32(nSamplesFromInput))
			psEnc.State_Fxx[0].SCmn.InputBufIx += nSamplesToBuffer
		}
		samplesIn = (*int16)(unsafe.Add(unsafe.Pointer(samplesIn), unsafe.Sizeof(int16(0))*uintptr(nSamplesFromInput*int(encControl.NChannelsAPI))))
