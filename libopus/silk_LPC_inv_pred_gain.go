...
func silk_LPC_inverse_pred_gain_c(A_Q12 *int16, order int) int32 {
	...
	for k = 0; k < order; k++ {
		DC_resp += int32(*(*int16)(unsafe.Add(unsafe.Pointer(A_Q12), unsafe.Sizeof(int16(0))*uintptr(k))))
		Atmp_QA[k] = int32(int(uint32(int32(*(*int16)(unsafe.Add(unsafe.Pointer(A_Q12), unsafe.Sizeof(int16(0))*uintptr(k)))))) << (int(QA - 12)))
	}
	if int(DC_resp) >= 4096 {
		return 0
	}
