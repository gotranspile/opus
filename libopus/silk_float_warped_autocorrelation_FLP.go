...
func silk_warped_autocorrelation_FLP(corr *float32, input *float32, warping float32, length int, order int) {
	...
	for n = 0; n < length; n++ {
		tmp1 = float64(*(*float32)(unsafe.Add(unsafe.Pointer(input), unsafe.Sizeof(float32(0))*uintptr(n))))
		for i = 0; i < order; i += 2 {
	...
	for i = 0; i < order+1; i++ {
		*(*float32)(unsafe.Add(unsafe.Pointer(corr), unsafe.Sizeof(float32(0))*uintptr(i))) = float32(C[i])
	}
}
