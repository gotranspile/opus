...
func silk_encode_indices(psEncC *silk_encoder_state, psRangeEnc *ec_enc, FrameIndex int, encode_LBRR int, condCoding int) {
	...
	ec_enc_icdf(psRangeEnc, int(psIndices.NLSFIndices[0]), []byte(&psEncC.PsNLSF_CB.CB1_iCDF[(int(psIndices.SignalType)>>1)*int(psEncC.PsNLSF_CB.NVectors)]), 8)
	...
	for i = 0; i < int(psEncC.PsNLSF_CB.Order); i++ {
		if int(psIndices.NLSFIndices[i+1]) >= NLSF_QUANT_MAX_AMPLITUDE {
			ec_enc_icdf(psRangeEnc, int(NLSF_QUANT_MAX_AMPLITUDE*2), []byte(&psEncC.PsNLSF_CB.Ec_iCDF[ec_ix[i]]), 8)
			...
			ec_enc_icdf(psRangeEnc, 0, []byte(&psEncC.PsNLSF_CB.Ec_iCDF[ec_ix[i]]), 8)
			ec_enc_icdf(psRangeEnc, int(-psIndices.NLSFIndices[i+1])-NLSF_QUANT_MAX_AMPLITUDE, silk_NLSF_EXT_iCDF[:], 8)
		} else {
			ec_enc_icdf(psRangeEnc, int(psIndices.NLSFIndices[i+1])+NLSF_QUANT_MAX_AMPLITUDE, []byte(&psEncC.PsNLSF_CB.Ec_iCDF[ec_ix[i]]), 8)
		}
	}
