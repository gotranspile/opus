...
func silk_encode_indices(psEncC *silk_encoder_state, psRangeEnc *ec_enc, FrameIndex int, encode_LBRR int, condCoding int) {
	...
	if condCoding == CODE_CONDITIONALLY {
		ec_enc_icdf(psRangeEnc, int(psIndices.GainsIndices[0]), &silk_delta_gain_iCDF[0], 8)
	} else {
		ec_enc_icdf(psRangeEnc, int(psIndices.GainsIndices[0])>>3, &silk_gain_iCDF[psIndices.SignalType][0], 8)
		ec_enc_icdf(psRangeEnc, int(psIndices.GainsIndices[0])&7, &silk_uniform8_iCDF[0], 8)
	}
	for i = 1; i < psEncC.Nb_subfr; i++ {
		ec_enc_icdf(psRangeEnc, int(psIndices.GainsIndices[i]), &silk_delta_gain_iCDF[0], 8)
	}
	ec_enc_icdf(psRangeEnc, int(psIndices.NLSFIndices[0]), (*uint8)(unsafe.Add(unsafe.Pointer(psEncC.PsNLSF_CB.CB1_iCDF), (int(psIndices.SignalType)>>1)*int(psEncC.PsNLSF_CB.NVectors))), 8)
	...
	for i = 0; i < int(psEncC.PsNLSF_CB.Order); i++ {
		if int(psIndices.NLSFIndices[i+1]) >= NLSF_QUANT_MAX_AMPLITUDE {
			ec_enc_icdf(psRangeEnc, int(NLSF_QUANT_MAX_AMPLITUDE*2), (*uint8)(unsafe.Add(unsafe.Pointer(psEncC.PsNLSF_CB.Ec_iCDF), ec_ix[i])), 8)
			ec_enc_icdf(psRangeEnc, int(psIndices.NLSFIndices[i+1])-NLSF_QUANT_MAX_AMPLITUDE, &silk_NLSF_EXT_iCDF[0], 8)
			...
			ec_enc_icdf(psRangeEnc, int(psIndices.NLSFIndices[i+1])+NLSF_QUANT_MAX_AMPLITUDE, (*uint8)(unsafe.Add(unsafe.Pointer(psEncC.PsNLSF_CB.Ec_iCDF), ec_ix[i])), 8)
		}
	}
	if psEncC.Nb_subfr == MAX_NB_SUBFR {
		ec_enc_icdf(psRangeEnc, int(psIndices.NLSFInterpCoef_Q2), &silk_NLSF_interpolation_factor_iCDF[0], 8)
	}
	if int(psIndices.SignalType) == TYPE_VOICED {
		...
		if encode_absolute_lagIndex != 0 {
			...
			ec_enc_icdf(psRangeEnc, int(pitch_high_bits), &silk_pitch_lag_iCDF[0], 8)
		...
		ec_enc_icdf(psRangeEnc, int(psIndices.PERIndex), &silk_LTP_per_index_iCDF[0], 8)
		for k = 0; k < psEncC.Nb_subfr; k++ {
			ec_enc_icdf(psRangeEnc, int(psIndices.LTPIndex[k]), silk_LTP_gain_iCDF_ptrs[psIndices.PERIndex], 8)
		}
		if condCoding == CODE_INDEPENDENTLY {
			ec_enc_icdf(psRangeEnc, int(psIndices.LTP_scaleIndex), &silk_LTPscale_iCDF[0], 8)
		}
	}
	psEncC.Ec_prevSignalType = int(psIndices.SignalType)
	ec_enc_icdf(psRangeEnc, int(psIndices.Seed), &silk_uniform4_iCDF[0], 8)
}
