...
func silk_DIV32_varQ(a32 int32, b32 int32, Qres int) int32 {
	...
	b32_inv = int32((int(silk_int32_MAX >> 2)) / (int(b32_nrm) >> 16))
	...
	if lshift < 0 {
		return int32(int(uint32(int32(func() int {
			if (int(math.MinInt32) >> (-lshift)) > (silk_int32_MAX >> (-lshift)) {
				if int(result) > (int(math.MinInt32) >> (-lshift)) {
					return int(math.MinInt32) >> (-lshift)
				}
				if int(result) < (silk_int32_MAX >> (-lshift)) {
					return silk_int32_MAX >> (-lshift)
				}
				return int(result)
			}
			if int(result) > (silk_int32_MAX >> (-lshift)) {
